...
| [homepass](https://github.com/ljmerza/homepass) | Home Assistant add-on serving time-limited guest links that control a chosen set of entities, with no HA account for the guest. Optional per-token PIN, scheduled start times, and a per-entity proximity requirement | ![Stars](https://img.shields.io/github/stars/ljmerza/homepass?style=flat-square) |
